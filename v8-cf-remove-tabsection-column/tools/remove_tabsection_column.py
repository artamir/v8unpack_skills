"""Remove a column (реквизит) from a tabular section of a catalog in a v8unpack source directory.

Usage:
    python remove_tabsection_column.py <source_dir> <catalog_name> <ts_name> <col_name>
...
Arguments:
    ...
    col_name      Name of the column to remove (e.g. "Реквизит1Строка10").

The tool modifies source_dir/Catalog/<catalog_name>/Catalog.json in-place:
  - Removes the column entry from the TS columns list (ts_entry[2])
  - Decrements the column count at ts_entry[2][1]
...
def remove_tabsection_column(source_dir, catalog_name, ts_name, col_name):
    """Remove a column from a tabular section in a catalog.
    ...
    found_idx = None
    found_uuid = None
    for i, entry in enumerate(dst_col_list[2:]):
        if _col_name(entry) == col_name:
            found_idx = i
            found_uuid = _col_uuid(entry)
            break

    if found_idx is None:
        raise ValueError(
            f'Column {col_name!r} not found in TS {ts_name!r} of {dst_catalog_json!r}')

    # Remove from list (offset by 2 for type_uuid and count)
    del dst_col_list[found_idx + 2]
    ...
    print(f'[remove_tabsection_column] Каталог {catalog_name!r}, ТЧ {ts_name!r}: '
          f'удалена колонка {col_name!r} (uuid={found_uuid}), осталось колонок: {count - 1}'
...
def _main():
    parser = argparse.ArgumentParser(
        description='Remove a column from a tabular section in a catalog.'
    )
    ...
    parser.add_argument('col_name', help='Name of the column to remove')
    args = parser.parse_args()
    remove_tabsection_column(args.source_dir, args.catalog_name,
