"""Add a column (реквизит) to a tabular section of a catalog in a v8unpack source directory.

The column definition is read from a reference catalog directory and inserted
into the target catalog's tabular section in Catalog.json.

Usage:
    python add_tabsection_column.py <source_dir> <catalog_name> <ts_name> <col_name> <col_source_catalog_dir>

Arguments:
    source_dir              Path to the v8unpack source directory to modify.
    catalog_name            Name of the catalog folder (e.g. "Справочник01").
    ts_name                 Name of the tabular section (e.g. "ТабличнаяЧасть1").
    col_name                Name of the column to add (e.g. "Реквизит1Строка10").
    col_source_catalog_dir  Directory containing the reference Catalog.json.

The tool modifies source_dir/Catalog/<catalog_name>/Catalog.json in-place:
  - Appends the column entry to the TS columns list (ts_entry[2])
  - Increments the column count at ts_entry[2][1]
...
def add_tabsection_column(source_dir, catalog_name, ts_name, col_name,
                          col_source_catalog_dir):
    """Add a column to a tabular section in a catalog.

    Args:
        source_dir:             Path to the source directory (modified in-place).
        catalog_name:           Name of the catalog folder (e.g. "Справочник01").
        ts_name:                Name of the tabular section.
        col_name:               Name of the column to add.
        col_source_catalog_dir: Directory containing the reference Catalog.json.
    """
    # ------------------------------------------------------------------
    # 1. Read column entry from source catalog dir
    # ------------------------------------------------------------------
    src_catalog_json = os.path.join(col_source_catalog_dir, CATALOG_JSON)
    if not os.path.isfile(src_catalog_json):
        raise FileNotFoundError(
            f'Catalog.json not found in {col_source_catalog_dir!r}')
    ...
    if src_ts_entry is None:
        raise ValueError(
            f'Tabular section {ts_name!r} not found in {src_catalog_json!r}')

    # Find the column in source TS
    src_col_list = src_ts_entry[2]
    col_entry = None
    for entry in src_col_list[2:]:
        if _col_name(entry) == col_name:
            col_entry = entry
            break

    if col_entry is None:
        raise ValueError(
            f'Column {col_name!r} not found in TS {ts_name!r} of {src_catalog_json!r}')

    col_uuid = _col_uuid(col_entry)

    # ------------------------------------------------------------------
    # 2. Insert column into target Catalog.json
    # ------------------------------------------------------------------
    ...
    if dst_ts_entry is None:
        raise ValueError(
            f'Tabular section {ts_name!r} not found in target {dst_catalog_json!r}')

    dst_col_list = dst_ts_entry[2]

    # Check if column already present
    for entry in dst_col_list[2:]:
        if _col_name(entry) == col_name:
            print(f'[add_tabsection_column] Колонка {col_name!r} уже присутствует, пропускаем.')
            return

    # Increment count and append entry
    count = int(str(dst_col_list[1]))
    dst_col_list[1] = str(count + 1)
    dst_col_list.append(col_entry)
    ...
    print(f'[add_tabsection_column] Каталог {catalog_name!r}, ТЧ {ts_name!r}: '
          f'добавлена колонка {col_name!r} (uuid={col_uuid}), всего колонок: {count + 1}'
...
def _main():
    parser = argparse.ArgumentParser(
        description='Add a column to a tabular section in a catalog.'
    )
    ...
    parser.add_argument('ts_name', help='Name of the tabular section')
    parser.add_argument('col_name', help='Name of the column to add')
    parser.add_argument('col_source_catalog_dir',
                        help='Directory containing the source Catalog.json')
    args = parser.parse_args()
    add_tabsection_column(args.source_dir, args.catalog_name,
                          args.ts_name, args.col_name,
                          args.col_source_catalog_dir)
